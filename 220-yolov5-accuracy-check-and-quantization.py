...
SAVE_DIR   = "./weights/yolov5s_openvino_model_quantization/"
...
class YOLOv5DataLoader(DataLoader):
    ...
    Create a class for the loading YOLOv5 dataset and annotation which inherits
    from POT API class DataLoader.The Ultralytics YOLOv5 training process requires
    image data normalization from [0,225] 8-bit integer range to [0.0,1.0] 32-bit
...
class COCOMetric(Metric):
    ...
    The COCOMetric.update() function contains post-processing with Non-Max Suppression
    to sort boxes by score and select the box with the highest score.
    """
...
def get_config():
    ...
    Here we use "DefaultQuantization" method for best performance of the quantization step.
    Alternatively, the "AccuracyAware" method can be used to keep accuracy loss below a
    predefined threshold, at the cost of performance during the quantization process
    (it takes longer to quantize the model). This is useful when DefaultQuantization
    results in more accuracy loss than desired.
    """

    config = dict()
    data_yaml = check_yaml(DATA_YAML)
    data = check_dataset(data_yaml, autodownload=False)
    ...
    algorithms = [
        {
            "name": "DefaultQuantization",  # DefaultQuantization or AccuracyAware
            "params": {
                    "target_device": "CPU", # CPU GPU ANY
                    "preset": "mixed",      # Choose between "mixed" and "performance"
                    "stat_subset_size": 300 # For production use, 300 is recommended
            }
        }
    ]
...
def quantization():
    ...
    The following 9 steps show how to quantize the model using the POT API.
    The optimized model and collected min-max values will be saved.

    It will take few minutes for generating FP32-INT8 model,
    please check the quantized model is successfully saved and
    you get the metric results of INT8 model.
    """
    ...
    optimized_save_dir = Path(save_dir)
    ...
    return metric_results_fp32, metric_results_i8


def draw_result(metric_results_fp32, metric_results_i8):
    ...
    # 保存图片
    plt.savefig(SAVE_DIR + "/Compare Yolov5 FP32 and INT8 model average precision.png")

    logger.info(f"Save precision compare image in {SAVE_DIR}")


if __name__ == "__main__":
    metric_results_fp32, metric_results_i8 = quantization()
    draw_result(metric_results_fp32, metric_results_i8)
