...
def coco2yolo():
    ...
    if tp == "bbox":
        ...
        segmentation(js["annotations"], images)
...
def bbox(annotations, images):
    ...
    print("=" * 100)
    print(
        f"总图片数量为 {num_pic}, 有bbox的图片数量为 {num_pic_bbox}, 没有bbox的图片数量为 {num_pic - num_pic_bbox}, 总bbox数为 {num_bbox}"
    )
    print("=" * 100)
...
def segmentation(annotations, images):
    for annotation in annotations:
        image_id = annotation["image_id"]
        category_id = annotation["category_id"]
        height = images[image_id]["height"]
    ...
    print("=" * 100)
    print(
        f"总图片数量为 {num_pic}, 有segmentation的图片数量为 {num_pic_segment}, 没有segmentation的图片数量为 {num_pic - num_pic_segment}, 总segmentation数为 {num_segment}"
    )
    print("=" * 100)
